...
RegClass(DiffList);
let $IS_CLIENT$ = true; // some functions check this if shared code on S and C/** Clone of some server object. */
    ...
    constructor(serverObjName){
        ...
        this.objProxy = new Proxy({}, {
            async get (target, key, receiver) {
                if (SELF.objCached[key] === null) SELF.objCached[key] = await rpc(`((k)=>(${SELF.serverObjName}[k]))`, key);
                return SELF.objCached[key];
            },
 ...
 */ class ProxyArraySetter {
    ...
    static __new(obj, field, existingArray = null) {
        let arr;
        ...
        Object.assign(arr, p);
        return Object.setPrototypeOf(arr, ProxyArraySetter.prototype);
    }
    push(...items) {
...
var _BLOCKS = {};
const BLOCKS = ServerProxyObject.new("BLOCKS");
var PAGES = {}; //all pages
var _TAGS = {};
const TAGS = ServerProxyObject.new("TAGS");
async function SaveAll() {
    TODO();
    Server.sendMsg({
        n: Server.MsgType.saveAll,
        d: {
            TAGS: _TAGS,
            BLOCKS: _BLOCKS
        }
...
async function rpc(code, ...fnArgs) {
    ...
    // }
    let resp = await Server.sendMsg({
        n: Server.MsgType.eval,
        d: code
    });
...
async function LoadInitial() {
    let initial = await rpc(`client_LoadInitial`);
...
class Block {
    ...
    __serialize__() {
        return Object.setPrototypeOf({
            ...
            /*serializer handles as normal array*/ tags: this._tags,
            /*serializer handles as normal array*/ attribs: this._attribs
        }, Block.prototype);
    }
    static __deserialize__(o) {
    ...
    async set(path, value, doAssign = true) {
        path = AttrPath.parse(path);
        console.log("SETTING ", path);
        let field = path[0];
        if (field.startsWith("_")) {
            if (doAssign) this[field] = value;
            field = field.substring(1);
            path[0] = field;
        } else {
            if (doAssign) {
                if (this['_' + field] !== undefined) this['_' + field] = value;
            }
        }
        return await rpc(`server_set`, [
            "BLOCKS",
            this.id,
            ...path
        ], value);
    ...
    static async new(text = "") {
        let b = await rpc(`Block.new`, text);
        _BLOCKS[b.id] = b;
        return b;
    }
    static async newPage(title = "") {
        let b = await rpc(`Block.newPage`, title);
        _BLOCKS[b.id] = b;
        PAGES[b.id] = true;
        return b;
    }
...
class Tag {
    ...
    static async new(name) {
        let t = await rpc(`Tag.new`, name);
        _TAGS[t.id] = t;
        if (t._parentTagId) (await TAGS[t._parentTagId])._blocks.push(t.id);
        return t;
    }
}
RegClass(Tag); /*
